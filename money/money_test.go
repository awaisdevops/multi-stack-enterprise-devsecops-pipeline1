...
func TestMultiplySlow(t *testing.T) {
	tests := []struct {
		...
		{"multiply by 1", mmc(5, 0, "USD"), 1, mmc(5, 0, "USD")},
		{"multiply by 2", mmc(5, 0, "USD"), 2, mmc(10, 0, "USD")},
		{"multiply by 3", mmc(2, 500000000, "EUR"), 3, mmc(7, 500000000, "EUR")},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := MultiplySlow(tt.m, tt.n)
			if !AreEquals(got, tt.want) {
				t.Errorf("MultiplySlow(%v, %d) = %v, want %v", tt.m, tt.n, got, tt.want)
			}
		})
	}
}
